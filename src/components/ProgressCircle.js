...
const CircularBar = styled.circle`
    ...
    animation: ${props => progressAnime(props.offset, props.circumference)} 2s linear forwards;
`
...
`
const ProgressCircle = ({ children, type, labelColor, size, progress, unit, strokeWidth, strokeShape, circleOneStroke, circleTwoStroke, ...rest }) => {
    ...
    <OuterCircle {...rest} size={size}>
      ...
      <CircularBar
        ...
        <SvgTextWrapper className="circle-label" center={center}>
          <SvgText x={center} y={center} className="circle-percentage" labelColor={labelColor}>
          {`${progress}${unit}`}
          </SvgText>
